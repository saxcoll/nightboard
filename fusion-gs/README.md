...
- For the finite-difference solver, the datasets, the surrogates, and the inverse model, $\psi = 0$ on the plasma boundary and $\psi_\mathrm{axis} \gt 0$ when $I_p \gt 0$. $\psi_n = 0$ on axis and $1$ on the boundary.
